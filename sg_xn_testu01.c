 ...
 * TestU01 Wrapper — XN Sequence
 * ============================================================
 *
 * This file implements the 2D-SG chaotic map and feeds the
 * xn sequence to the TestU01 statistical test suite.
 *
 * MAP EQUATIONS:
 ...
 *
 * COMPILE:
 *   gcc -O3 -o sg_xn_testu01 sg_xn_testu01.c \
 *       -lm -ltestu01 -lprobdist -lmylib
 *
 * USAGE:
 *   ./sg_xn_testu01 smallcrush 1e13
 *   ./sg_xn_testu01 crush      1e13
 *   ./sg_xn_testu01 bigcrush   1e13
 *
 * PARAMETERS (fixed):
 *   c1 = 3.7,  c2 = 6.3
...
/* ============================================================
 * XN SEQUENCE — 32-BIT OUTPUT WORD
 *
 ...
 *   w  = interleave64(ux XOR vy, uy XOR vx)
 *   w  = mix64(w)
 *   r  = upper 32 bits of w
 * ============================================================ */
static unsigned int xn_next_u32(void) {
    ...
    /* Cross-combine xn and yn values */
    uint64_t w = interleave64(ux ^ vy, uy ^ vx);
...
int main(int argc, char *argv[]) {
    if (argc < 3) {
        fprintf(stderr,
            ...
            "Examples:\n"
            "  ./sg_xn_testu01 smallcrush 1e13\n"
            "  ./sg_xn_testu01 crush      1e14\n"
            "  ./sg_xn_testu01 bigcrush   1e15\n",
            argv[0]);
        return 1;
    }
    ...
    printf("=== 2D-SG Chaotic Map — XN Sequence ===\n");
    printf("Test      : %s\n", test);
    printf("Bit limit : %.2e\n", bits_d);
    printf("c1=%.1f  c2=%.1f\n", gc1, gc2);
    printf("x0=%.15f\n", gx);
